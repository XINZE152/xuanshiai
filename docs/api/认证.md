...
| `face_verified` | integer | 人脸认证状态，`0` 未认证、`1` 通过、`2` 失败；取自 `user_auth.face_verified` |
| `need_bind_phone` | boolean | 是否需要绑定手机号 |

变更记录（2026-09-01）：响应新增必返字段 `face_verified`。这是向后兼容的响应扩展，旧客户端可忽略该字段；需要人脸认证门禁的客户端应仅在值为 `1` 时视为认证通过。

#### 开发/测试环境手动调整人脸认证状态

人脸认证状态保存在 `user_auth.face_verified`：`0` 未认证、`1` 认证通过、`2` 认证失败。先按用户 ID 查询并确认目标账号，再在开发或测试数据库执行更新：

```sql
SELECT user_id, face_verified FROM user_auth WHERE user_id = 123;
UPDATE user_auth SET face_verified = 1 WHERE user_id = 123;
```

生产环境不得通过前端参数或临时 SQL 直接伪造通过状态；应由人脸认证供应商回调或具备权限的后台审核流程写入，并保留审核人、时间和审计记录。当前项目尚未接入真实人脸供应商，测试时可使用上述方式验证门禁和 `auth/me` 返回值。
