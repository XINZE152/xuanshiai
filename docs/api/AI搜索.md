...
接口前缀：`/api/v1/ai`。本文件是 M03 AI 搜索（统一方案 §8，执行计划 Task 10）对外的完整契约。

> **命名说明（2026-09-11）：墨相师（Moxiang）就是 AI 画像功能的产品更名**；本文件中的「画像数据/画像投影」均指墨相师旅程建构并发布的画像。旧对话式画像 REST 问答接口（`/ai/profile-sessions*`）已删除。
自然语言只编译为 Task 1 冻结的 10 个 allowlist 字段，再由服务端静态映射落到现有结构化筛选；模型输出永远不能成为 SQL、列名、`ORDER BY` 或表名。
