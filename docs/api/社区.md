...
| `status` | string | 是 | 不为空 | 初始为 `ready`；发布绑定后为 `bound` |
| `moderation_status` | string | 是 | 不为空 | 审核状态：`pending` 审核中、`approved` 可发布、`rejected` 拒绝、`hidden` 隐藏；development/testing 上传后为 `approved`，staging/production 初始为 `pending` |
...
发布规则：`image_media_ids` 和 `video_media_id` 仅可使用当前用户本人、用途匹配、媒体类型匹配、未删除、`status=ready` 且 `moderation_status=approved` 的媒体。生产或预发布环境收到 `pending` 时，前端应轮询下方查询接口，审核通过后再发动态；不要以固定延时替代状态判断。
...
#### `GET /api/v1/community/media/{media_id}`

- 权限：已登录、绑定手机号且已实名认证；仅返回当前用户未删除的媒体。
- 请求体：无。
- 成功：`200 OK`，返回结构与上传响应 `CommunityMediaResponse` 完全一致。
- 路径参数：`media_id` 为正整数；非法示例 `0` 返回 `422`。
- `404`：媒体不存在、已删除或不属于当前用户；前端应停止轮询并移除该媒体。

调用示例：

```http
GET /api/v1/community/media/42 HTTP/1.1
Authorization: Bearer <access_token>
...
审核状态独立于媒体生命周期。`pending` 媒体仍为 `ready`，但不可绑定；审核通过后保持 `ready` 并可发布。审核拒绝或隐藏的媒体不可发布。
