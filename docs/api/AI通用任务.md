...
接口前缀：`/api/v1`。本文件是 AI 画像（墨相师）、搜索与匹配度后台通用任务（`ai_task` 事实源，统一方案 §6.4 / §11.3）对外的查询与取消契约。**命名说明：墨相师（Moxiang）就是 AI 画像功能的产品更名**；旧 `profile_extract` 问答任务已于 2026-09-11 删除，现存任务类型见各能力文档。业务任务由 Task 7+ 各自接口创建；本文件的通用接口只负责「查看任务状态与安全结果引用」和「在可取消窗口内取消任务」。
