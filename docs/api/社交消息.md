...
- 聊天发送支持可选 body 字段 `client_message_id`（新客户端文字消息应始终提供）；同一发送者、会话、ID 及相同载荷的重试返回原消息，不带 ID 的旧客户端维持原有非幂等行为。发布内容和举报没有本次新增的幂等键；这些写请求仍应按各自接口规则处理超时。
...
权限：登录且手机号已绑定；服务端按当前用户身份返回可见会话。成功状态 `200 OK`，按置顶及最近消息时间倒序排列。
...
会话仅包含仍匹配、未互相拉黑且对当前用户未隐藏的会话。响应是分页对象，不是裸数组：`items`、`page`、`page_size`、`total`、`has_more`。
...
| `last_message_time` | datetime/null | 是 | 尚未发消息时 `null` | 最后一条消息时间 |
| `unread_count` | integer | 是 | 无未读时为 `0` | 当前用户未读消息数 |

分页字段：`page` 为当前页，`page_size` 为请求的每页数量，`total` 为总会话数，`has_more` 表示是否还有下一页。
...
```json
{"items":[{"id":8,"target":{"user_id":23,"nickname":"小雨","avatar":"/storage/uploads/23/avatar.webp","age":28},"last_message":"你好，很高兴认识你","last_message_time":"2026-07-20T10:30:00","unread_count":2}],"page":1,"page_size":20,"total":1,"has_more":false}
```

没有会话时返回 `{"items":[],"page":1,"page_size":20,"total":0,"has_more":false}`。后续聊天路径必须使用 `items[].id`，不能将 `target.user_id` 当作会话 ID。
...
| `content` | string/null | 是 | 媒体消息可为 `null`；撤回后为 `消息已撤回` | 文本、卡片或系统消息内容 |
| `media_url` | string/null | 是 | 非媒体消息或撤回后为 `null` | 已上传媒体地址 |
| `client_message_id` | string/null | 是 | 历史消息或无幂等键发送为 `null` | 发送该消息时的客户端幂等键 |
| `is_read` | boolean | 是 | 不为空 | 是否已读 |
| `revoked` | boolean | 是 | 不为空 | 是否已撤回 |
| `is_recalled` | boolean | 是 | 不为空 | 撤回状态兼容字段 |
| `recalled_at` | datetime/null | 是 | 未撤回为 `null` | 撤回时间 |
...
需要登录并完成手机号绑定（Bearer access token）。会话必须属于当前用户、双方仍有有效匹配（状态 `1/2`）且未互相拉黑；服务端还检查发送限制及接收方的消息隐私设置。接收方启用认证限制而发送方未通过实名认证时，返回 `403`。成功状态 `201 Created`；媒体上传仍由既有上传接口处理，本接口只接收已上传的 `media_url`。本次仅修改既有发送路由，没有新增 HTTP 路由。
...
| --- | --- | --- | --- | --- | --- | --- |
| `session_id` | path | integer | 是 | 无 | `>=1`；必须是当前用户参与且仍有权限的会话 | 聊天会话 ID |
| `type` | body | integer | 否 | `1` | `1~6` | `1` 文本、`2` 图片、`3` 语音、`4` 视频、`5` 小程序卡片、`6` 系统消息 |
| `content` | body | string/null | 条件必填 | `null` | 最长 5000；`type=1` 必填；`type=5/6` 时 content/media_url 至少一项 | 文本、卡片或系统消息内容 |
| `media_url` | body | string/null | 条件必填 | `null` | 最长 500；`type=2/3/4` 必填；`type=5/6` 时 content/media_url 至少一项 | 已上传图片、语音、视频或卡片媒体地址 |
| `client_message_id` | body | string/null | 否（新客户端文字消息应提供） | `null` | 最长 128 字符；服务端 trim 首尾空白；给出时不得为空白 | 客户端为一条逻辑消息生成并在重试时复用的幂等键 |

文本请求示例（同一逻辑消息的网络重试必须复用同一个 ID）：

```http
POST /api/v1/chat/sessions/8/messages
Authorization: Bearer <access_token>
Content-Type: application/json
```

```json
{"type":1,"content":"你好，很高兴认识你","client_message_id":"msg-2026-0001"}
```

旧客户端可不传 `client_message_id`，仍按原有行为每次创建一条消息；新客户端对每条普通文字消息都应传稳定 ID。图片示例仍可使用 `{"type":2,"media_url":"/storage/uploads/chat/message.webp"}`，任何消息类型传入 ID 时均适用下述幂等规则。

返回参数：成功返回单个 `ChatMessageResponse`，字段为：

| 字段 | 类型 | 必返 | 空值含义 | 含义 |
| --- | --- | --- | --- | --- |
| `id` | integer | 是 | 不为空 | 服务端消息 ID；重放返回原 ID |
| `session_id` | integer | 是 | 不为空 | 聊天会话 ID |
| `from_user_id` | integer | 是 | 不为空 | 发送者 ID |
| `to_user_id` | integer | 是 | 不为空 | 接收者 ID |
| `type` | integer | 是 | `1~6` | 消息类型，含义同请求 |
| `content` | string/null | 是 | 媒体消息可为 `null`；撤回后为 `消息已撤回` | 文本、卡片或系统消息内容 |
| `media_url` | string/null | 是 | 无媒体或已撤回时为 `null` | 已上传媒体地址 |
| `client_message_id` | string/null | 是 | 旧消息或无幂等键发送为 `null` | 本次消息的幂等键；重放回显原键 |
| `is_read` | boolean | 是 | 不为空 | 是否已读 |
| `revoked` | boolean | 是 | 不为空 | 是否已撤回 |
| `is_recalled` | boolean | 是 | 不为空 | 撤回状态兼容字段 |
| `recalled_at` | datetime/null | 是 | 未撤回为 `null` | 撤回时间 |
| `created_at` | datetime | 是 | 不为空 | 原消息创建时间；重放不变 |

成功响应示例（首次发送及相同内容重放均为 HTTP `201`，重放的对象与原消息相同）：

```json
{"id":101,"session_id":8,"from_user_id":23,"to_user_id":41,"type":1,"content":"你好，很高兴认识你","media_url":null,"client_message_id":"msg-2026-0001","is_read":false,"revoked":false,"is_recalled":false,"recalled_at":null,"created_at":"2026-09-23T10:00:00"}
```

幂等与重试：唯一范围是 `(from_user_id, session_id, client_message_id)`，不同用户或不同会话可使用相同 ID。相同范围内 ID 相同且 `type`、`content`、`media_url` 全部相同，服务端返回原消息，不重复插入、不更新会话摘要/未读数、不重复发通知；同一 ID 的任一载荷字段不同，返回 `409 Conflict`，错误体为 `{"detail":"client_message_id 已用于不同的消息内容"}`。新消息先生成并持久化一个 ID，网络超时后使用原 ID 和原请求体重试；不要为重试生成新 ID，也不要用冲突响应重试不同内容。服务端在幂等查询前仍重新执行限制、会话成员/匹配/拉黑和接收方隐私校验，权限撤销后重试仍会被拒绝。未提供 ID 的旧客户端不享受幂等保证。
...
无请求参数，成功状态 `200 OK`。不存在设置记录时返回默认值。资料可见性、消息权限、请求/响应示例及错误契约见 `docs/api/设置.md`（2026-09-04 变更）。
...
| `profile_visibility` | string/null | 否 | `all` | `all`/`friends`/`only_me` | 资料对所有人、已匹配用户或仅自己可见 |
| `message_privacy` | string/null | 否 | `all` | `all`/`friends`/`certified` | 接收消息权限；`certified` 要求发送方已实名认证 |
...
| `409` | 业务状态冲突，或同一 client_message_id 被用于不同载荷 | `client_message_id 已用于不同的消息内容` | 载荷冲突时不要继续复用该 ID；其他业务冲突重新查询状态 |
| `410` | 调用已废弃的 `GET /relations/liked-by` | `鍠滄鍒楄〃浠呮湰浜哄彲瑙?` | 移除该入口；仅使用 `GET /relations/likes` |
| `422` | 类型、长度、范围或枚举不合法（包括 client_message_id 为空白或超长） | `client_message_id 不能是空白` | 修正请求参数 |
...
- 既有 `POST /api/v1/chat/sessions/{session_id}/messages` 请求新增可选 `client_message_id`（trim 后最长 128 字符，纯空白拒绝）；旧客户端可省略，新客户端文字消息应始终提供。消息写入响应及历史消息对象新增同名字段，旧行/无键消息返回 `null`。
- 唯一范围为 `(from_user_id, session_id, client_message_id)`。相同 ID 且 `type/content/media_url` 相同的重试返回原消息，不重复写消息、会话摘要、未读计数或通知；同 ID 不同载荷返回 `409`。权限检查仍在重放查询前执行。未提供 ID 的旧客户端保留原行为。
- 数据库升级、验证和回滚见 `migrations/chat/README.md`；本次没有新增 HTTP 路由。

### 2026-09-04（设置页隐私权限）

- **新增字段：** `GET/PUT/PATCH /api/v1/users/me/privacy` 新增 `profile_visibility` 和 `message_privacy`，默认均为 `all`；响应新增 `updated_at`。
- **方法兼容：** 新增部分更新方法 `PATCH`，保留既有 `PUT`，请求体和响应完全一致，旧客户端无需迁移。
- **服务端执行：** `friends` 仅对 `user_match.status IN (1,2)` 的双方开放资料；`only_me` 不进入推荐且不能被其他用户访问；`certified` 与既有 `only_auth_can_contact` 均在发送消息时校验发送方实名认证。
