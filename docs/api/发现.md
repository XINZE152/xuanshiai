...
- 2026-08-17：推荐不再排除仅浏览过的用户；只有明确划过的用户继续排除，已提交认识申请或已匹配的对象仍可在首页推荐中看到，避免申请后双方失去入口。
...
### 变更记录（education_levels 方向修正）

- 2026-09-13：`education_levels` 由 1=博士…5=高中（与存储域相反）修正为与 `user_profile.education_level` 存储域一致的 1=高中及以下…5=博士。此前以该端点为字典源构造 `education_min` 会得到反向筛选结果（"本科及以上"实际筛出"本科及以下"）。当前前端未消费该端点（搜索页映射使用档案编辑同源字典），无调用方迁移需求。
...
```json
{
  ...
  "education_levels":[{"value":1,"label":"高中及以下"},{"value":3,"label":"本科"}],
  "cities":["北京","上海","杭州"]
}
```
...
- 排除：自己、未完善用户、不可见用户、互相拉黑和明确划过的用户。提交认识申请后，申请中、已通过或已匹配的对象仍可继续在首页看到；已浏览但未明确划过的用户也可再次推荐。
