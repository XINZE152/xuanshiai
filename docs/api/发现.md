...
- 排除：自己、未完善用户、不可见用户、互相拉黑、已划过、申请中和已匹配用户。已浏览但未明确划过的用户仍可再次推荐，适用于小用户池场景。
- 兜底：严格按择偶偏好筛选无结果时，自动放宽择偶偏好条件，在其余可见性和关系过滤不变的前提下随机推荐候选用户；此时名片 `match_score` 为 `0`，`match_reason` 为 `随机推荐`。仍无候选时返回空列表。
