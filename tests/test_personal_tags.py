...
from app.core.profile_tags import LEGACY_TAG_CATEGORIES, TAG_CATEGORIES, TAG_CATALOG_REVISION, personal_tags, split_personal_tags
...
def test_catalog_only_describes_the_person_and_has_unique_options():
    ...
    assert len(options) == len({option.strip().casefold() for option in options}) == 268
    assert [len(options) for _, _, options in TAG_CATEGORIES] == [32, 30, 12, 11, 13, 14, 9, 22, 25, 10, 19, 10, 9, 9, 9, 12, 22]
    assert {option for _, _, group in LEGACY_TAG_CATEGORIES for option in group} <= set(options)
...
@pytest.mark.parametrize("tags", [[], ["阅读"], ["阅读", "有幽默感"], list(TAG_CATEGORIES[1][2][:10])])
...
def test_custom_tags_are_normalized_and_share_the_total_limit():
    request = ProfileUpdateRequest(personal_tags=["阅读", "  手碟  ", "城市 骑行", "livehouse"])
    assert request.personal_tags == ["阅读", "手碟", "城市 骑行", "Livehouse"]
    assert personal_tags(request.personal_tags, ["手碟", "城市 骑行"]) == request.personal_tags
    assert personal_tags(request.personal_tags) == ["阅读", "Livehouse"]
...
def test_invalid_custom_tags_are_rejected(tags):
    with pytest.raises(ValidationError):
        ProfileUpdateRequest(personal_tags=tags)
...
def test_legacy_read_deduplicates_and_reclassifies_without_inference():
    ...
    assert personal_tags(values) == ["有幽默感", "阅读", "温柔细心", "健身"]
    assert split_personal_tags(values) == (["阅读", "健身"], ["有幽默感", "温柔细心"])
    assert row["tags"]["city"] == ["上海"]


@pytest.mark.asyncio
@pytest.mark.parametrize("selected", [[], ["阅读", "有幽默感"], list(TAG_CATEGORIES[1][2][:10]), ["熟人面前话多", "云吸猫", "周末煲汤", "Livehouse"]])
...
async def test_catalog_version_and_limits():
    ...
    assert result.catalog_revision == "2026-09-10.4"
...
async def test_custom_write_is_moderated_and_persisted_in_explicit_group(monkeypatch):
    ...
    await profile.update_profile(db, 42, ProfileUpdateRequest(personal_tags=selected))
...
async def test_rejected_custom_tag_is_not_written(monkeypatch):
    ...
    with pytest.raises(Exception) as exc:
        await profile.update_profile(db, 42, ProfileUpdateRequest(personal_tags=["手碟"]))
...
def test_custom_tags_reach_discovery_only_with_explicit_storage_marker():
    marked = {"interest_tags": '["手碟"]', "personality_tags": "[]", "tags": {"custom": ["手碟"]}}
...
async def test_read_preserves_legacy_only_for_owner_and_respects_privacy(monkeypatch, public, locked):
    ...
    assert data["legacy_tags"] == ([] if public else ["寻找长期伴侣", "上海"])
    assert data["personal_tags"] == ([] if locked else ["阅读", "有幽默感", "熟人面前话多", "云吸猫"])
    if locked:
...
def test_new_tags_reach_discovery_and_recommendation_without_leaking_locked_details():
    row = {"user_id": 42, "interest_tags": '["Livehouse","云吸猫","本科"]',
           "personality_tags": '["熟人面前话多"]', "tags": {"food_lifestyle": ["周末煲汤"]}}
    assert _all_tags(row) == {"Livehouse", "云吸猫", "熟人面前话多", "周末煲汤"}
    card = _card(row, 50, "资料匹配")
    assert card.personal_tags == ["Livehouse", "云吸猫", "熟人面前话多"]
    assert card.interest_tags == ["Livehouse", "云吸猫"]
...
async def test_completion_threshold_counts_distinct_valid_tags_across_categories(count):
    ...
    selected = ["熟人面前话多", "云吸猫", "周末煲汤", "Livehouse", "普拉提", "看日出", "拼图", "AI工具", "爵士", "手账"][:count]
    interests, personalities = split_personal_tags(selected)
