...
# ---------------------------------------------------------------------------
# Phase 2 Task 6：memory 投影读取接入（candidate_filter/personal_profile）
# ---------------------------------------------------------------------------


def _structured_key(subject: str, dimension: str, field_key: str) -> str:
...
async def _memory_projection_store():
    ...
    for uid in (501, 502, 503):
        ...
        for category in ("personal_profile", "ideal_partner_preference"):
            store.grants[(uid, "search", "candidate_filter", category)] = {
    ...
    # 501：personal_profile（structured height_cm）+ ideal_partner_preference
    seed_claim(
        store,
        "c-501-h",
        owner_user_id=501,
        canonical_key=_structured_key("personal", "lifestyle", "height_cm"),
        value=175,
    )
    seed_claim(
        store,
    ...
    # 502：只有 ideal_partner_preference 投影（无个人资料）
    seed_claim(
        store,
        "c-502-ideal",
        owner_user_id=502,
    ...
    for kwargs in (
        {"owner_user_id": 501, "data_category": "personal_profile"},
        {"owner_user_id": 501, "data_category": "ideal_partner_preference"},
        {"owner_user_id": 502, "data_category": "ideal_partner_preference"},
    ):
        await service.build(function_key="search", purpose="candidate_filter", **kwargs)
    return session, store


@pytest.mark.asyncio
async def test_memory_mode_load_projections_uses_personal_profile_entries(
    monkeypatch,
) -> None:
    monkeypatch.setattr(settings, "ai_memory_projection_read_mode", "memory")
    session, _store = await _memory_projection_store()
    result = await search_mod._load_projections(session, [501, 502, 503])
    # 501 有 personal_profile；502 只有 ideal_partner 投影；503 全缺。
    assert 501 in result and 502 not in result and 503 not in result, (
        "只有 personal_profile 投影可作候选人资料；ideal_partner 不得误用"
    )
    fields = result[501]["fields"]
    assert fields.get("height_cm") == 175, "entries 编译为 fields dict"
    assert all("希望对方" not in str(value) for value in fields.values()), (
        "ideal_partner 偏好原文不得进入候选人资料"
    )
    assert result[501]["source"] == "memory_projection"


@pytest.mark.asyncio
async def test_memory_mode_missing_projection_excludes_candidate(
    monkeypatch,
) -> None:
    monkeypatch.setattr(settings, "ai_memory_projection_read_mode", "memory")
    session, _store = await _memory_projection_store()
    result = await search_mod._load_projections(session, [503])
    assert 503 not in result, "缺投影的候选人不进入结果（不扩大数据范围）"


@pytest.mark.asyncio
async def test_shadow_mode_search_dual_read_returns_legacy(
    monkeypatch, caplog
) -> None:
    ...
    legacy = {
        501: {"id": 7, "fields": {"height_cm": 175}, "source_hash": "h"},
        503: {"id": 9, "fields": {"age": 30}, "source_hash": "h2"},
    }

    async def _fake_legacy(db, user_ids):
        return {uid: legacy[uid] for uid in user_ids if uid in legacy}

    monkeypatch.setattr(search_mod, "_load_legacy_projections", _fake_legacy)
    with caplog.at_level(_logging.INFO, logger="app.services.ai.search"):
        result = await search_mod._load_projections(session, [501, 503])
    ...
    assert diff_lines, "shadow 必须记录 diff"
    joined = " ".join(diff_lines)
    assert "age" in joined and "identical=True" in joined, (
        "501 两侧一致（只记 identical），503 记录差异字段 age"
    )
    assert "我每天" not in joined, "diff 日志不得携带字段原文"


@pytest.mark.asyncio
async def test_compiled_filters_only_use_server_side_fields() -> None:
    """模型输出只能编译为服务端字段（现有编译器契约的显式回归）。"""
    from app.schemas.discovery import DiscoveryFilters

    allowed = set(DiscoveryFilters.model_fields)
    assert {"gender", "age_min", "age_max", "city_code", "education_min"} <= allowed
    # LLM 输出里的未知字段不会出现在 DiscoveryFilters（extra 行为由编译器控制）
    filters = DiscoveryFilters(age_min=25, age_max=35)
    dumped = filters.model_dump(exclude_none=True)
    assert {k: v for k, v in dumped.items() if k.startswith("age")} == {
        "age_min": 25,
        "age_max": 35,
    }
